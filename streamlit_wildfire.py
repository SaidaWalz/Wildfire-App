...
# Flow: Click map → H3 r5 snap → fetch ERA5-Land (last 12 days) → LSTM inference → show result
#
# Requirements:
#   pip install streamlit streamlit-folium folium h3 cdsapi xarray numpy pandas tensorflow
#
...
#
# Model file:
#   saved_model/lstm_model.keras   (input_shape=(12,7), output sigmoid or softmax-2)
...
# ─────────────────────────────────────────────────────────
H3_RES        = 5
SEQ_LEN       = 12          # days back
N_FEATURES    = 7
WILDFIRE_IDX  = 1           # index in softmax-2 output (ignored for sigmoid)
THRESHOLD     = 0.5

MODEL_PATH    = Path("saved_model") / "lstm_model.keras"
...
def load_model():
    ...
    if not MODEL_PATH.exists():
        raise FileNotFoundError(f"Model not found: {MODEL_PATH.resolve()}\n"
                                f"Place your .keras file at {MODEL_PATH}")
...
def run_lstm(model, scaler, df: pd.DataFrame) -> float:
    """df shape (12, 7) → scalar fire probability."""
    ...
    x = np.expand_dims(x, axis=0)   # (1, 12, 7)
    y = np.array(model.predict(x, verbose=0))

    if y.ndim == 2 and y.shape[1] == 2:
        return float(y[0, WILDFIRE_IDX])
...
def h3_polygon_coords(cell: str):
    """Return folium-compatible [(lat,lon), ...] boundary."""
